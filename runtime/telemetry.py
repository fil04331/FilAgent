...
class TelemetryConfig(BaseModel):
    ...
    tracing: Dict[str, Any] = Field(
        default_factory=lambda: {"enabled": True, "sampler": {"type": "always_on", "ratio": 1.0}}
    )
    exporter: Dict[str, Any] = Field(
        default_factory=lambda: {
            "type": "jaeger",
            "jaeger": {
    ...
    batch_processor: Dict[str, Any] = Field(
        default_factory=lambda: {
            "enabled": True,
            "max_queue_size": 2048,
    ...
    privacy: Dict[str, Any] = Field(
        default_factory=lambda: {"mask_pii": True, "hash_user_ids": True}
    )
    ...
    def load_config(self, config_path: Optional[str] = None) -> TelemetryConfig:
        ...
        try:
            ...
            if "environment" in service_config:
                service_config["environment"] = os.getenv(
                    "FILAGENT_ENV", service_config["environment"]
                )
    ...
    def initialize(self, config_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if exporter:
                ...
                if self._config.batch_processor.get("enabled", True):
                    processor = BatchSpanProcessor(
                        ...
                        schedule_delay_millis=self._config.batch_processor.get(
                            "schedule_delay_millis", 5000
                        ),
                        max_export_batch_size=self._config.batch_processor.get(
                            "max_export_batch_size", 512
                        ),
                        export_timeout_millis=self._config.batch_processor.get(
                            "export_timeout_millis", 30000
                        ),
                    )
                else:
            ...
            self._tracer = trace.get_tracer(
                self._config.service["name"], self._config.service["version"]
            )
