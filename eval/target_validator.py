...
class EvaluationTarget:
    ...
    def __post_init__(self):
        ...
        if self.operator not in [op.value for op in ComparisonOperator]:
            raise ValueError(f"Invalid operator: {self.operator}. Must be one of {[op.value for op in ComparisonOperator]}")
    ...
    def _parse_benchmarks(benchmarks: Dict[str, Any]) -> List[EvaluationTarget]:
        ...
        for benchmark_name, benchmark_config in benchmarks.items():
            ...
            for metric, value in benchmark_config.items():
                if metric in ['enabled', 'time_limit_seconds', 'temperature', 'max_tokens', 'tasks_to_run']:
                    continue
...
class TargetValidator:
    ...
    def validate(self, benchmark_results: Dict[str, Dict[str, float]]) -> Dict[str, Any]:
        ...
        for target in self.targets:
            current_value = self._get_metric_value(benchmark_results, target.benchmark, target.metric)
    ...
    def print_report(self, validation_results: Dict[str, Any]) -> None:
        ...
        print("\n" + "-"*80)
        print(f"SUMMARY: {validation_results['passed_count']}/{validation_results['total_count']} targets passed")
...
def validate_from_config(
    benchmark_results: Dict[str, Dict[str, float]],
    config_path: str = "config/eval_targets.yaml"
) -> Dict[str, Any]:
    """
