...
class PlanValidator:
    ...
    def parse_plan_from_text(text: str) -> List[Task]:
        ...
        task_pattern = re.compile(
            r"^\s*(?:(?:Step|Task|Étape)\s+(\d+|[A-Za-z])|(\d+)\.)\s*[:\-]?\s*(.+)", re.IGNORECASE
        )
        ...
        if not tasks and text.strip():
            ...
            for i, sentence in enumerate(sentences[:10]):  # Max 10 steps
                if len(sentence) > 10:  # Meaningful sentence
                    ...
                    tasks.append(
                        Task(
                            id=f"task_{i+1}", name=sentence, dependencies=dependencies, duration=1.0
                        )
                    )
    ...
    def simulate_execution(tasks: List[Task]) -> Dict[str, Any]:
        ...
        """
        if not tasks:
            return {"success": False, "error": "empty_plan", "completed": set()}
    ...
    def _compute_task_levels(tasks: List[Task]) -> Dict[str, int]:
        ...
        def compute_level(task_id: str) -> int:
            ...
            max_dep_level = max(
                compute_level(dep_id) for dep_id in task.dependencies if dep_id in task_dict
            )
...
def evaluate_planning_capability(plan_text: str) -> Dict[str, Any]:
    ...
    if not tasks:
        return {"passed": False, "error": "No tasks found in plan", "tasks": []}
    ...
    return {
        ...
        "tasks": [
            {"id": t.id, "name": t.name, "dependencies": list(t.dependencies)} for t in tasks
        ],
        "execution": execution,
        "quality": quality,
    }
