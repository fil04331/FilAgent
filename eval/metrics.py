...
class MetricsAggregator:
    ...
    def collect_historical_data(
        self, days: int = 30, benchmark_name: Optional[str] = None
    ) -> List[ReportData]:
        """
    ...
    def compute_trend(
        self, benchmark_name: str, metric_name: str = "pass_at_1", days: int = 30
    ) -> TrendData:
        """
    ...
    def print_dashboard(self, dashboard: DashboardData) -> None:
        ...
        if benchmarks is not None and isinstance(benchmarks, dict):
            ...
            for benchmark_name, trend in benchmarks.items():
                ...
                print(
                    f"{benchmark_name:20s}: {latest_val*100:5.1f}% (avg: {mean_val*100:5.1f}%) {indicator}{warning}"
                )
...
def main() -> None:
    ...
    parser.add_argument(
        "--benchmark", type=str, default=None, help="Specific benchmark to analyze (default: all)"
    )
