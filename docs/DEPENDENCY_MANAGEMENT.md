...
For backward compatibility with pip-based workflows:
...
# Export all dependencies (complete)
pdm export --prod -G dev -G ml -G ui -G benchmark -o requirements_complete.txt --without-hashes
```

⚠️ **CRITICAL**: The `requirements*.txt` files are **auto-generated** by PDM. Never edit them manually!
- These files are marked with `# This file is @generated by PDM.` header
- They must be regenerated after every dependency change
- Manual edits will be overwritten on next regeneration
- Dependency conflicts often arise from stale generated files
