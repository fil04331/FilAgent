...
class ABTestRunner:
    ...
    def analyze_results(self):
        ...
        if len(self.metrics[self.version_a]) > 0 and len(self.metrics[self.version_b]) > 0:
            a_clarity = sum(m.clarity_score for m in self.metrics[self.version_a]) / len(
                self.metrics[self.version_a]
            )
            b_clarity = sum(m.clarity_score for m in self.metrics[self.version_b]) / len(
                self.metrics[self.version_b]
            )
