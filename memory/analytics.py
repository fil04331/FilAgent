...
def create_tables():
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_conversation_id ON interaction_logs(conversation_id)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_feedback_score ON interaction_logs(user_feedback_score)"
    )
