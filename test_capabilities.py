...
class FilAgentCapabilityTester:
    ...
    def test_tools_execution(self) -> bool:
        ...
        tools_tests = [
            {"name": "calculator", "prompt": "Calcule 25 * 4 + 10", "expected": "110"},
            {
                "name": "python_sandbox",
                "prompt": "Exécute ce code Python: print('Hello FilAgent')",
                "expected": "Hello FilAgent",
            },
            {"name": "file_reader", "prompt": f"Lis le fichier README.md", "expected": "LLM-Agent"},
        ]
    ...
    def test_memory_system(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{API_URL}/conversations/{self.conversation_id}-memory", timeout=5
            )
    ...
    def test_quebec_specific_features(self) -> bool:
        ...
        try:
            ...
            response = requests.post(
                ...
                json={
                    "messages": [
                        {"role": "user", "content": "Calcule les taxes sur 1000$ avec TPS et TVQ"}
                    ],
    ...
    def generate_report(self):
        ...
        functional_capabilities = sum(
            1 for v in self.results["capabilities"].values() if v and v != "error"
        )

        # Score global
        score = (
            (functional_capabilities / total_capabilities * 100) if total_capabilities > 0 else 0
        )
        ...
        report_path = (
            PROJECT_ROOT / f"capability_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
