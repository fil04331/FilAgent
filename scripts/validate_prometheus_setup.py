    ...
    def validate_python_dependencies(self):
        ...
        try:
            ...
            self.results.append(ValidationResult(
                "requests",
                False,
                "Non installé (requis pour les tests)"
            ))
        ...
        for file_path, description in required_files:
            ...
            if full_path.exists():
                ...
                self.results.append(ValidationResult(
                    file_path,
                    True,
                    f"{description} présent ({size} bytes)"
                ))
            else:
                self.results.append(ValidationResult(
                    file_path,
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.results.append(ValidationResult(
                    "Server /health",
                    False,
                    f"Retourne code {response.status_code}"
                ))
    ...
    def validate_metrics_endpoint(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                is_prometheus_format = "text/plain" in content_type or "prometheus" in content_type.lower()
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "success":
                    ...
                    self.results.append(ValidationResult(
                        "Prometheus API",
                        False,
                        f"Retourne status: {data.get('status')}"
                    ))
            else:
                self.results.append(ValidationResult(
                    "Prometheus API",
    ...
    def validate_alert_rules(self):
        ...
        if not alerts_file.exists():
            self.results.append(ValidationResult(
                "Alert rules file",
                False,
                "Fichier prometheus_alerts.yml non trouvé"
            ))
            return
        ...
        try:
            ...
            if not alerts_config or "groups" not in alerts_config:
                self.results.append(ValidationResult(
                    "Alert rules format",
                    False,
                    "Format YAML invalide ou pas de groupes"
                ))
                return
...
def main():
    ...
    parser.add_argument(
        "--check-prometheus",
        action="store_true",
        help="Vérifier la connexion à Prometheus"
    )
    
    parser.add_argument(
        "--check-alerts",
        action="store_true",
        help="Vérifier les règles d'alertes"
    )
    ...
    success = validator.validate(
        check_prometheus=args.check_prometheus,
