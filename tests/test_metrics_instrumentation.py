...
class TestComplianceGuardianMetrics:
    ...
    def test_compliance_guardian_records_forbidden_pattern_metrics(self, mock_metrics):
        ...
        try:
            guardian.validate_query("How to hack the system", context={"user_id": "test_user"})
        except Exception:
            pass
    ...
    def test_compliance_guardian_records_pii_detection(self, mock_metrics):
        ...
        result = guardian.validate_query(
            "My email is test@example.com", context={"user_id": "test_user"}
        )
...
class TestToolExecutorMetrics:
    ...
    def test_tool_executor_records_execution_metrics(self, mock_metrics):
        ...
        mock_tool.execute = Mock(return_value=ToolResult(status=ToolStatus.SUCCESS, output="42"))
    ...
    def test_tool_executor_records_error_status(self, mock_metrics):
        ...
        mock_tool.execute = Mock(
            return_value=ToolResult(
                status=ToolStatus.ERROR, output="", error="Tool execution failed"
            )
        )
...
class TestMetricsEndToEnd:
    ...
    @pytest.mark.skipif(
        True, reason="Requires full agent setup"  # Skip by default as it requires full system
    )
...
class TestNoPIIInInstrumentation:
    ...
    def test_query_content_not_in_metrics(self, mock_metrics):
        ...
        result = guardian.validate_query(sensitive_query, context={"user_id": "test_user"})
    ...
    def test_tool_arguments_not_in_metrics(self, mock_metrics):
        ...
        mock_tool.validate_arguments = Mock(return_value=(True, None))
        mock_tool.execute = Mock(
            return_value=ToolResult(status=ToolStatus.SUCCESS, output="result")
        )
        ...
        tool_call = ToolCall(tool="test_tool", arguments={"password": sensitive_data})
