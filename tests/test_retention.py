...
def create_old_provenance(temp_logs_structure):
    ...
    def _create(days_old: int, count: int = 1):
        ...
        for i in range(count):
            ...
            prov_data = {
                "trace_id": f"trace-{file_index:04d}",
                "entities": [],
                "activities": []
            }
...
@pytest.mark.unit
@pytest.mark.parametrize("ttl_days,timestamp_days_old,expected_expired", [
...
def test_cleanup_events_empty_directory(temp_retention_config, temp_logs_structure, monkeypatch):
    ...
    monkeypatch.setattr('memory.retention.Path', lambda x: temp_logs_structure['events'] if x == "logs/events" else Path(x))
...
@pytest.mark.unit
def test_cleanup_events_no_old_files(temp_retention_config, temp_logs_structure, create_old_event_logs, monkeypatch):
    ...
    # Patcher le chemin des logs
    events_dir = temp_logs_structure['events']
    monkeypatch.setattr('memory.retention.Path', lambda x: events_dir if x == "logs/events" else Path(x))
...
@pytest.mark.unit
def test_cleanup_events_with_old_files(temp_retention_config, temp_logs_structure, create_old_event_logs, monkeypatch):
...
def test_cleanup_events_missing_directory(temp_retention_config, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('memory.retention.Path', lambda x: nonexistent_dir if x == "logs/events" else Path(x))
...
@pytest.mark.unit
def test_cleanup_decisions_with_old_files(temp_retention_config, temp_logs_structure, create_old_decisions, monkeypatch):
    ...
    # Patcher le chemin des logs
    decisions_dir = temp_logs_structure['decisions']
    monkeypatch.setattr('memory.retention.Path', lambda x: decisions_dir if x == "logs/decisions" else Path(x))
...
@pytest.mark.unit
def test_cleanup_provenance_with_old_files(temp_retention_config, temp_logs_structure, create_old_provenance, monkeypatch):
    ...
    # Patcher le chemin des logs
    monkeypatch.setattr('memory.retention.Path', lambda x: provenance_dir if x == "logs/traces/otlp" else Path(x))
...
@pytest.mark.unit
def test_cleanup_events_with_malformed_filename(temp_retention_config, temp_logs_structure, monkeypatch):
...
@pytest.mark.unit
def test_cleanup_decisions_missing_timestamp(temp_retention_config, temp_logs_structure, monkeypatch):
...
def test_full_retention_workflow(temp_retention_config, temp_logs_structure, create_old_event_logs, create_old_decisions, create_old_provenance, monkeypatch):
