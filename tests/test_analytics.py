...
def test_create_analytics_tables(temp_analytics_db):
    ...
    cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_feedback_score'"
    )
    assert cursor.fetchone() is not None
...
def test_add_interaction_log_minimal(temp_analytics_db):
    ...
    with patch("memory.analytics.DB_PATH", temp_analytics_db):
        add_interaction_log(conversation_id="test-conv-2", user_feedback_score=3)
...
def test_add_interaction_log_invalid_score(temp_analytics_db):
    ...
    with patch("memory.analytics.DB_PATH", temp_analytics_db):
        ...
        with pytest.raises(ValueError, match="must be between 1 and 5"):
            add_interaction_log(conversation_id="test-conv-3", user_feedback_score=0)

        # Score too high
        with pytest.raises(ValueError, match="must be between 1 and 5"):
            add_interaction_log(conversation_id="test-conv-3", user_feedback_score=6)
...
def test_interaction_log_metadata(temp_analytics_db):
    ...
    with patch("memory.analytics.DB_PATH", temp_analytics_db):
        ...
        add_interaction_log(conversation_id="conv-meta", user_feedback_score=5, metadata=metadata)
