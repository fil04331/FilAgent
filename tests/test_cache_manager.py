...
def mock_dependencies():
    ...
    with patch.dict("sys.modules", {"faiss": mock_faiss, "sentence_transformers": mock_st_class}):
...
def mock_faiss_index():
    ...
    index.search = MagicMock(
        return_value=(np.array([[0.95]]), np.array([[0]]))  # High similarity  # First entry
    )
    return index
...
class TestCacheSimilarity:
    ...
    def test_cache_hit_similar_query(self, cache_manager):
        ...
        cache_manager.index.search = MagicMock(
            return_value=(
...
class TestCacheMetrics:
    ...
    def test_hit_rate_calculation(self, cache_manager):
        ...
        # Store entries
        cache_manager.store(query="Query 1", response_text="Response 1", conversation_id="conv_1")

        cache_manager.store(query="Query 2", response_text="Response 2", conversation_id="conv_2")

        # Mock searches - 1 hit, 2 misses
        # First query: hit
        cache_manager.index.search = MagicMock(return_value=(np.array([[0.95]]), np.array([[0]])))
        result1 = cache_manager.get("Query 1")
        assert result1 is not None

        # Second query: miss (low similarity)
        cache_manager.index.search = MagicMock(return_value=(np.array([[0.5]]), np.array([[0]])))
        result2 = cache_manager.get("Different query")
        assert result2 is None

        # Third query: miss (empty result)
        cache_manager.index.search = MagicMock(return_value=(np.array([[]]), np.array([[]])))
    ...
    def test_hit_count_increments(self, cache_manager):
        ...
        # Store entry
        cache_manager.store(
            query="Popular query", response_text="Popular response", conversation_id="conv_1"
        )
...
class TestCacheInvalidation:
    ...
    def test_invalidate_by_age(self, cache_manager):
        ...
        with patch("memory.cache_manager.datetime") as mock_datetime:
            ...
            cache_manager.store(
                query="Old query", response_text="Old response", conversation_id="conv_old"
            )
            ...
            cache_manager.store(
                query="Recent query", response_text="Recent response", conversation_id="conv_recent"
            )
    ...
    def test_invalidate_all(self, cache_manager):
        ...
        for i in range(5):
            cache_manager.store(
                query=f"Query {i}", response_text=f"Response {i}", conversation_id=f"conv_{i}"
            )
...
class TestCacheEviction:
    ...
    def test_evict_oldest_on_max_size(self, cache_manager):
        ...
        cache_manager.store(
            query="Query 10", response_text="Response 10", conversation_id="conv_10"
        )
    ...
    def test_save_and_load_cache(self, temp_cache_paths, mock_faiss_index, mock_embedder):
        ...
        with patch("memory.cache_manager.SentenceTransformer", return_value=mock_embedder):
            with patch("memory.cache_manager.faiss") as mock_faiss:
                ...
                cache1.store(
                    query="Test query", response_text="Test response", conversation_id="conv_test"
                )
...
class TestCacheEdgeCases:
    ...
    def test_override_similarity_threshold(self, cache_manager):
        ...
        cache_manager.store(
            query="Test query", response_text="Test response", conversation_id="conv_1"
        )

        # Mock search with moderate similarity
        cache_manager.index.search = MagicMock(
            return_value=(np.array([[0.85]]), np.array([[0]]))  # Below default threshold (0.9)
        )
        ...
        with patch("memory.cache_manager.FAISS_AVAILABLE", True):
            with patch("memory.cache_manager.SENTENCE_TRANSFORMERS_AVAILABLE", True):
                with patch("memory.cache_manager.SemanticCacheManager") as MockCache:
                    ...
                    init_cache_manager(similarity_threshold=0.95, max_cache_size=500, ttl_hours=48)
