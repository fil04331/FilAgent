        ...
        if script_path.exists():
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    f"import sys; sys.path.insert(0, '{SCRIPTS_DIR.parent}'); exec(open('{script_path}').read().split('if __name__')[0])",
                ],
                capture_output=True,
                text=True,
                timeout=5,
...
class TestScriptsErrorHandling:
    ...
    def test_validate_prometheus_setup_no_dependencies(self):
        ...
        if script_path.exists():
            ...
            assert (
                "requests" in result.stdout.lower()
                or "requests" in result.stderr.lower()
