...
def test_submit_feedback_nonexistent_conversation(client, temp_db, temp_analytics_db):
    ...
    with patch('memory.episodic.DB_PATH', temp_db), \
        ...
        response = client.post(
            "/api/v1/feedback/nonexistent-conv",
            json={"score": 5}
        )
...
@pytest.mark.integration
def test_submit_multiple_feedback_same_conversation(client, temp_db, temp_analytics_db, setup_conversation):
...
@pytest.mark.integration
def test_submit_feedback_background_task_failure(client, temp_db, temp_analytics_db, setup_conversation):
