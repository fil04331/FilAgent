...
class TaskExecutor:
    ...
    def execute(
        ...
        try:
            ...
            raise ExecutionError(
                f"Execution failed critically: {str(e)}"
            ) from e
    ...
    def _execute_task(self, task: Task) -> Any:
        ...
        if task.action not in self.actions:
            raise ExecutionError(
                f"Unknown action '{task.action}' for task {task.task_id}"
            )
        ...
        try:
            ...
            raise ExecutionError(
                f"Action '{task.action}' failed: {error_type}: {error_msg}"
            ) from e
    ...
    def _propagate_failure(self, failed_task: Task, graph: TaskGraph):
        ...
        for task in to_skip:
            if task.status == TaskStatus.PENDING:
                task.update_status(
                    TaskStatus.SKIPPED,
                    f"Dependency {failed_task.task_id} failed"
                )
